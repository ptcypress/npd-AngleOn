...
if not os.path.exists(csv_path):
    ...
    if 'Brush' not in df.columns or 'Pressure' not in df.columns or 'Velocity' not in df.columns:
        ...
        moves objects faster under identical conditions, especially at lower pressures (lower object weight ex. Nuts, Bolts, Washers, etc.). For both products, once objects exerted approx. 3 lbs/in²,  
        the feeder coil could no longer generate the force required for movement. All testing was performed using 36" linear vibratory feeder @ 120VAC. Please reference the chart below for more detail. 
